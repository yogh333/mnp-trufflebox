...
contract("Board - Related to random", async (accounts) => {
  ...
  describe("B. Test of the fulfillRandomness function", () => {
    ...
    xit("8. Emits an event after the release of the random number", async function () {
      ...
      await BoardInstance.register(new BN(0), new BN(1));
      expectEvent(
        await BoardInstance.rawFulfillRandomness(requestId, randomness),
        "RandomReady",
        { requestId: web3.utils.hexToBytes(requestId) }
      );
    });
  });
  ...
  /*describe("C. Test of the function play", () => {
    beforeEach("SETUP", async () => {
      ...
      await BoardInstance.getFaucet(
        BoardInstance.address,
        web3.utils.toBN(new BN(60).mul(new BN(10).pow(new BN(28))))
      );
    });
    ...
    it("11. Test random number related to the movement of the pawn", async () => {
      ...
      const oldPositionPawn = await BoardInstance.getPawnInfo(
        new BN(0),
        new BN(1)
      );
      ...
      let newPositionPawn = await BoardInstance.getPawnInfo(
        new BN(0),
        new BN(1)
      );
      ...
      assert.strictEqual(
        parseInt(newPositionPawn[1]),
        parseInt(oldPositionPawn[1]) + random + 2
      );
    });
    ...
    it("12.a. We test the modulos and the movement of the pawn -- random = 0", async () => {
      ...
      let newPositionPawn2 = await BoardInstance.getPawnInfo(
        new BN(0),
        new BN(1)
      );

      assert.strictEqual(
        parseInt(newPositionPawn2[1]),
        parseInt(newPositionPawnProcessed)
      );
    });
